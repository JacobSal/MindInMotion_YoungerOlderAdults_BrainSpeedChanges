...
for(cond in CONDITIONS){
  for(plotBand_cort in c("theta","alpha","beta","all")){
    ...
    netData_sbjs_base = readMat(paste(mat_dir,"netVals_",BASE_COND,"_aveTime_sbjs.mat",sep=""))
    ...
    for(i in cli){
      for(j in clj){
        ...
        if(i!=j){
          counter=counter+1
          val_in_1 = netData_sbjs$SAVEVAR[[1]][[counter_all]][[1]] - netData_sbjs_base$SAVEVAR[[1]][[counter_all]][[1]];
          val_in_2 = netData_sbjs$SAVEVAR[[2]][[counter_all]][[1]] - netData_sbjs_base$SAVEVAR[[2]][[counter_all]][[1]]
          val_in_3 = netData_sbjs$SAVEVAR[[4]][[counter_all]][[1]] - netData_sbjs_base$SAVEVAR[[4]][[counter_all]][[1]]
          val_in_4 = netData_sbjs$SAVEVAR[[3]][[counter_all]][[1]] - netData_sbjs_base$SAVEVAR[[3]][[counter_all]][[1]]
          pvals_theta[counter] = t.test(val_in_1)$p.value
          pvals_alpha[counter] = t.test(val_in_2)$p.value
          pvals_all[counter] = t.test(val_in_3)$p.value
          pvals_beta[counter] = t.test(val_in_4)$p.value
        }
      }
    }
    ...
    for(i in cli){
      for(j in clj){
        if(i!=j){
          ...
          pvals_beta_box[j,i] = pvals_all[counter]
        }
      }
    }
    ...
    netData_ave_base = readMat(paste(mat_dir,"netVals_",BASE_COND,"_aveTime.mat",sep=""))
    ...
    #Load base condition and sig mask
    thetaAve_base = netData_ave_base$SAVEVAR[[1]]
    alphaAve_base = netData_ave_base$SAVEVAR[[2]]
    allAve_base = netData_ave_base$SAVEVAR[[4]]
    betaAve_base = netData_ave_base$SAVEVAR[[3]]
    thetaAve_base[isSig_theta_box==0]=0
    alphaAve_base[isSig_alpha_box==0]=0
    allAve_base[isSig_all_box==0]=0
    betaAve_base[isSig_all_box==0]=0
    # subtract
    thetaAve = thetaAve - thetaAve_base;
    alphaAve = alphaAve - alphaAve_base;
    allAve = allAve - allAve_base;
    betaAve = betaAve - betaAve_base;
    ...
    cortAll = alphaAve[1:length(sub_ints),1:length(sub_ints)]
    cortBeta = alphaAve[1:length(sub_ints),1:length(sub_ints)]
    ...
    for(i in 1:length(sub_ints)) {
      for(j in 1:length(sub_ints)){
        if((A[i,j]!=0 | A[j,i]!=0) & (A[j,i]>A[i,j])){
          if(sign_vals[i,j]==0){
            if(plotBand_cort=='theta'){
              data_temp = abs(netData_ave$SAVEVAR[[1]]-netData_ave_base$SAVEVAR[[1]])*MULTI_FACTOR
              circos.link(cluster_names[i], c(curr_starts_gray[i],curr_starts_gray[i]+data_temp[i,j]), cluster_names[j], c(curr_starts_gray[j],curr_starts_gray[j]+data_temp[j,i]), col = adjustcolor('gray', alpha.f = 0.3)) 
            }else if(plotBand_cort=='alpha'){
              data_temp = abs(netData_ave$SAVEVAR[[2]]-netData_ave_base$SAVEVAR[[2]])*MULTI_FACTOR
              circos.link(cluster_names[i], c(curr_starts_gray[i],curr_starts_gray[i]+data_temp[i,j]), cluster_names[j], c(curr_starts_gray[j],curr_starts_gray[j]+data_temp[j,i]), col = adjustcolor('gray', alpha.f = 0.3)) 
            }else if(plotBand_cort=='all'){
              data_temp = abs(netData_ave$SAVEVAR[[4]]-netData_ave_base$SAVEVAR[[4]])*MULTI_FACTOR
              circos.link(cluster_names[i], c(curr_starts_gray[i],curr_starts_gray[i]+data_temp[i,j]), cluster_names[j], c(curr_starts_gray[j],curr_starts_gray[j]+data_temp[j,i]), col = adjustcolor('gray', alpha.f = 0.3)) 
            }else if(plotBand_cort=='beta'){
              data_temp = abs(netData_ave$SAVEVAR[[3]]-netData_ave_base$SAVEVAR[[3]])*MULTI_FACTOR
              circos.link(cluster_names[i], c(curr_starts_gray[i],curr_starts_gray[i]+data_temp[i,j]), cluster_names[j], c(curr_starts_gray[j],curr_starts_gray[j]+data_temp[j,i]), col = adjustcolor('gray', alpha.f = 0.3)) 
            }
      ...
      for(j in 1:length(sub_ints)){
        if((A[i,j]!=0 | A[j,i]!=0) & (A[j,i]>A[i,j])){
          if(sign_vals[i,j]>0 & sign_vals[j,i]>0){
            circos.link(cluster_names[i], c(curr_starts[i],curr_starts[i]+A[i,j]), cluster_names[j], c(curr_starts[j],curr_starts[j]+A[j,i]), col = adjustcolor('red', alpha.f = 0.6))#color_vals_links[8]) 
            curr_starts[i] = curr_starts[i]+A[i,j]
            curr_starts[j] = curr_starts[j]+A[j,i]
          }else if(sign_vals[i,j]<0 & sign_vals[j,i]<0){
            circos.link(cluster_names[i], c(curr_starts[i],curr_starts[i]+A[i,j]), cluster_names[j], c(curr_starts[j],curr_starts[j]+A[j,i]), col = adjustcolor('blue', alpha.f = 0.6)) 
            curr_starts[i] = curr_starts[i]+A[i,j]
            curr_starts[j] = curr_starts[j]+A[j,i]
          }else if(sign_vals[i,j]!=0 & sign_vals[j,i]!=0){
            circos.link(cluster_names[i], c(curr_starts[i],curr_starts[i]+A[i,j]), cluster_names[j], c(curr_starts[j],curr_starts[j]+A[j,i]), col = adjustcolor('magenta', alpha.f = 0.6)) 
            curr_starts[i] = curr_starts[i]+A[i,j]
            curr_starts[j] = curr_starts[j]+A[j,i]
          }
        }
