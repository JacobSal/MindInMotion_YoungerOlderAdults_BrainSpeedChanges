...
% sbatch /blue/dferris/jsalminen/GitHub/par_EEGProcessing/src/3_ANALYZE/MIM_YA/run_b_ic_clustering_expanded.sh
...
%     tmp_group_unif{subj_i} = 'Young Adults';
% end
...
%                 'values2', COND_DESIGNS{des_i});
    ...
    for des_i = 1:length(COND_DESIGNS)
        ...
        %## conditions across all younger subjects
        group_only_flag = false;
        [STUDY] = std_makedesign(STUDY, ALLEEG, des_i,...
            'subjselect', {ALLEEG.subject},...
            'variable1',COND_EVENT_CHAR,...
            'values1',COND_DESIGNS{des_i});
       if ~exist([cluster_dir filesep 'commonbase_nobasenorm_wicondbase'],'dir')
           mkdir([cluster_dir filesep 'commonbase_nobasenorm_wicondbase'])
       end
...
%             stacked_cond_ersp = [allersp{:}];
    ...
    %             tmp_std = std(allersp{cond_i},[],3);
            ...
            fprintf('Plotting Per Subject ERSP...\n');
            [pcond_ersp,pgroup_ersp,pinter_ersp,stat_cond,stat_group,stat_inter] = std_stat(allersp,...
                    ...
                    'fieldtripmethod',FIELDTRIP_METHOD,'fieldtripmcorrect',ERSP_MCORRECT,'fieldtripnaccu',ERSP_NACCU);
            %- plot
            fprintf('Plotting Using std_plottf.m\n')
...
%             saveas(fig_i,[cluster_dir filesep sprintf('ersp_%i_%s.fig',cluster_i,[COND_DESIGNS{des_i}{:}])]);
            saveas(fig_i,[cluster_dir filesep 'commonbase_nobasenorm_wicondbase' filesep sprintf('allsub_commonbase_nobasenorm_wicondbase_ersp_%i_%s.jpg',cluster_i,[COND_DESIGNS{des_i}{:}])]);
            close(fig_i)
            
            %## SINGLE SUBJECT LOOP
            fprintf('Plotting Per Subject ERSP...\n');
            for subj_i = 1:size(allersp{1},3)
                tmp_ersp = cell(length(allersp),1);
                for cond_i = 1:length(allersp)
                ...
                fig_i.Name = sprintf('Cluster %i) ERSP averages across Conditions',cluster_i);
    %             saveas(fig_i,[cluster_dir filesep sprintf('ersp_%i_%s.fig',cluster_i,[COND_DESIGNS{des_i}{:}])]);
                saveas(fig_i,[cluster_dir filesep 'commonbase_nobasenorm_wicondbase' filesep sprintf('%s_commonbase_nobasenorm_wicondbase_ersp_%i_%s.jpg',ALLEEG(subj_i).subject,cluster_i,[COND_DESIGNS{des_i}{:}])]);
                close(fig_i)
            end
    ...
    %% ASSIGN GROUP NAME & STUDY STATS (COMMONBASE,BASENORM,WITHINCOND BASE)
    STUDY = pop_statparams(STUDY,'condstats',ERSP_CONDSTATS,...
            'groupstats',ERSP_GROUPSTATS,...
            'method',ERSP_STAT_METHOD,...
            'singletrials',ERSP_SINGLETRIALS,'mode',STAT_MODE,'fieldtripalpha',ERSP_ALPHA,...
    ...
    for des_i = 1:length(COND_DESIGNS)
        ...
        [STUDY] = std_makedesign(STUDY, ALLEEG, des_i,...
            ...
            'variable1',COND_EVENT_CHAR,...
            'values1',COND_DESIGNS{des_i});
       if ~exist([cluster_dir filesep 'commonbase_basenorm_wicondbase'],'dir')
           mkdir([cluster_dir filesep 'commonbase_basenorm_wicondbase'])
       end
       parfor (j = 1:length(valid_cluster),POOL_SIZE)
%         for j = 1:length(valid_cluster)
            cluster_i = valid_cluster(j);
...
%             stacked_cond_ersp = [allersp{:}];
    ...
    %             tmp_std = std(allersp{cond_i},[],3);
            ...
            fig_i.Name = sprintf('Cluster %i) ERSP averages across Conditions',cluster_i);
%             saveas(fig_i,[cluster_dir filesep sprintf('ersp_%i_%s.fig',cluster_i,[COND_DESIGNS{des_i}{:}])]);
            saveas(fig_i,[cluster_dir filesep 'commonbase_basenorm_wicondbase' filesep sprintf('allsub_commonbase_basenorm_wicondbase_ersp_%i_%s.jpg',cluster_i,[COND_DESIGNS{des_i}{:}])]);
            close(fig_i)
            ...
            for subj_i = 1:size(allersp{1},3)
                ...
                fig_i.Name = sprintf('Cluster %i) ERSP averages across Conditions',cluster_i);
    %             saveas(fig_i,[cluster_dir filesep sprintf('ersp_%i_%s.fig',cluster_i,[COND_DESIGNS{des_i}{:}])]);
                saveas(fig_i,[cluster_dir filesep 'commonbase_basenorm_wicondbase' filesep sprintf('%s_commonbase_basenorm_wicondbase_ersp_%i_%s.jpg',ALLEEG(subj_i).subject,cluster_i,[COND_DESIGNS{des_i}{:}])]);
                close(fig_i)
            end
    ...
    %% ASSIGN GROUP NAME & STUDY STATS (NO COMMONBASE,NO BASENORM,WITHINCOND BASE)
    STUDY = pop_statparams(STUDY,'condstats',ERSP_CONDSTATS,...
            'groupstats',ERSP_GROUPSTATS,...
            'method',ERSP_STAT_METHOD,...
            'singletrials',ERSP_SINGLETRIALS,'mode',STAT_MODE,'fieldtripalpha',ERSP_ALPHA,...
    ...
    for des_i = 1:length(COND_DESIGNS)
        ...
        [STUDY] = std_makedesign(STUDY, ALLEEG, des_i,...
            ...
            'variable1',COND_EVENT_CHAR,...
            'values1',COND_DESIGNS{des_i});
       if ~exist([cluster_dir filesep 'nocommonbase_nobasenorm_wicondbase'],'dir')
           mkdir([cluster_dir filesep 'nocommonbase_nobasenorm_wicondbase'])
       end
       parfor (j = 1:length(valid_cluster),POOL_SIZE)
%         for j = 1:length(valid_cluster)
            cluster_i = valid_cluster(j);
...
%             stacked_cond_ersp = [allersp{:}];
    ...
    %             tmp_std = std(allersp{cond_i},[],3);
            ...
            fig_i.Name = sprintf('Cluster %i) ERSP averages across Conditions',cluster_i);
%             saveas(fig_i,[cluster_dir filesep sprintf('ersp_%i_%s.fig',cluster_i,[COND_DESIGNS{des_i}{:}])]);
            saveas(fig_i,[cluster_dir filesep 'nocommonbase_nobasenorm_wicondbase' filesep sprintf('allsub_nocommonbase_nobasenorm_wicondbase_ersp_%i_%s.jpg',cluster_i,[COND_DESIGNS{des_i}{:}])]);
            close(fig_i)
            ...
            for subj_i = 1:size(allersp{1},3)
                ...
                fig_i.Name = sprintf('Cluster %i) ERSP averages across Conditions',cluster_i);
    %             saveas(fig_i,[cluster_dir filesep sprintf('ersp_%i_%s.fig',cluster_i,[COND_DESIGNS{des_i}{:}])]);
                saveas(fig_i,[cluster_dir filesep 'nocommonbase_nobasenorm_wicondbase' filesep sprintf('%s_nocommonbase_nobasenorm_wicondbase_ersp_%i_%s.jpg',ALLEEG(subj_i).subject,cluster_i,[COND_DESIGNS{des_i}{:}])]);
                close(fig_i)
            end
    ...
    for des_i = 1:length(COND_DESIGNS)
        ...
        [STUDY] = std_makedesign(STUDY, ALLEEG, des_i,...
            ...
            'variable1',COND_EVENT_CHAR,...
            'values1',COND_DESIGNS{des_i});
       if ~exist([cluster_dir filesep 'commonbase_nobasenorm_nowicondbase'],'dir')
           mkdir([cluster_dir filesep 'commonbase_nobasenorm_nowicondbase'])
       end
       parfor (j = 1:length(valid_cluster),POOL_SIZE)
%         for j = 1:length(valid_cluster)
            cluster_i = valid_cluster(j);
...
%             stacked_cond_ersp = [allersp{:}];
            ...
            fig_i.Name = sprintf('Cluster %i) ERSP averages across Conditions',cluster_i);
%             saveas(fig_i,[cluster_dir filesep sprintf('ersp_%i_%s.fig',cluster_i,[COND_DESIGNS{des_i}{:}])]);
            saveas(fig_i,[cluster_dir filesep 'commonbase_nobasenorm_nowicondbase' filesep sprintf('allsub_commonbase_nobasenorm_wicondbase_ersp_%i_%s.jpg',cluster_i,[COND_DESIGNS{des_i}{:}])]);
            close(fig_i)
            ...
            for subj_i = 1:size(allersp{1},3)
                ...
                fig_i.Name = sprintf('Cluster %i) ERSP averages across Conditions',cluster_i);
    %             saveas(fig_i,[cluster_dir filesep sprintf('ersp_%i_%s.fig',cluster_i,[COND_DESIGNS{des_i}{:}])]);
                saveas(fig_i,[cluster_dir filesep 'commonbase_nobasenorm_nowicondbase' filesep sprintf('%s_commonbase_nobasenorm_wicondbase_ersp_%i_%s.jpg',ALLEEG(subj_i).subject,cluster_i,[COND_DESIGNS{des_i}{:}])]);
                close(fig_i)
            end
...
for j = 1:4
    allerspdata_meanSubj{j,1}(:,:,:) = zeros(size(allersp{j},1),size(allersp{j},2),length(subj_in_cluster));
end
...
for n = 1:length(unique(STUDY.cluster(cluster_i).sets))    
    ...
    if cluster_i == 1 %bad comp
        for j = 1:4
%                     keyboard
            allerspdata_remove{j}(:,:,p) = nan(size(allersp{j},1),size(allersp{j},2),1);
