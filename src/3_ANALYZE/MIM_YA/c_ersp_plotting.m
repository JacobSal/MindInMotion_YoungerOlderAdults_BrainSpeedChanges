...
%- run .sh
% sbatch /blue/dferris/jsalminen/GitHub/par_EEGProcessing/src/3_ANALYZE/MIM/run_c_ersp_plotting.sh
...
%         grandavg_timwarp = [0,262,706,981,1414];
        for i = [2,4,5,6]
%             axes(fig_i.Children(2));
            ...
            set(tmp,'XTick',grandavg_timwarp)
            xx = num2cell(grandavg_timwarp);
            xx = cellfun(@num2str,xx,'UniformOutput',false);
%             set(tmp,'XTickLabel',xx)
            set(tmp,'XTickLabel',{'RHS', 'LTO', 'LHS', 'RTO', 'RHS'});
            xlabel(tmp,'Time (ms)','Fontsize',16,'fontweight','bold');
            xline(tmp,grandavg_timwarp(1),'k--');
            xline(tmp,grandavg_timwarp(2),'k--');
            xline(tmp,grandavg_timwarp(3),'k--');
            xline(tmp,grandavg_timwarp(4),'k--');
            xline(tmp,grandavg_timwarp(5),'k--');
        end
        %{
