...
group_id = cell(1,length([SUBJ_ITERS{:}]));
stack_iter = 0;
...
%         disp(SUBJ_PICS{group_i}{subj_i})
        subject_chars{cnt} = SUBJ_PICS{group_i}{subj_i};
        group_id{cnt} = group_i; %GROUP_NAMES{group_i};
        cnt = cnt + 1;
    end
...
fPaths = fPaths(inds);
fNames = fNames(inds);
subject_chars = subject_chars(inds);
group_id = group_id(inds);
        ...
        for cond_i = 1:length(condition_gait)
            ...
            for cl_i = 1:length(TMP_STUDY.cluster)
                ...
                if ~isempty(comp_i) && length(comp_i) < 2
                    fprintf('%s) assigning cluster %i in condition %s...\n',subject_chars{subj_i},cl_i,condition_gait{cond_i});
