...
parfor (subj_i = 1:length(subject_chars),floor(length(subject_chars)/3))
    ...
    if ~exist([STUDY_DIR filesep EEG_BASE.subject filesep spca_params_tmp.condition_base],'dir')
        mkdir([STUDY_DIR filesep EEG_BASE.subject filesep spca_params_tmp.condition_base]);
    end
    pop_saveset(EEG_BASE,'filepath',[STUDY_DIR filesep EEG_BASE.subject filesep spca_params_tmp.condition_base],...
...
%     base_mean = output_struct.baseline_ersp;
    for cond_i = 1:length(spca_params_tmp.condition_gait)
        ...
        if ~isempty(fpath)
            exportgraphics(fig,[fpath filesep sprintf('chan%i_cond%s_ersp_corr.jpg',CHAN_INT,condition,SPCA_PARAMS.condition_gait{cond_i})]);
        end
        %##
        fig = plot_txf(squeeze(GPM_corr(:,CHAN_INT,:)),[],WAVELET_STRUCT.f);
