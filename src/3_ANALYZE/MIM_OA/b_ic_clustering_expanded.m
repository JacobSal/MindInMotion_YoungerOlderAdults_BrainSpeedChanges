...
%     saveas(fig_i,[cluster_dir filesep 'Cluster_dipole_seperatepanes_avg.fig']);
    ...
    std_dipplot_CL(STUDY,ALLEEG,'clusters',CLUSTER_PICKS,'figure','off','mode','together_averaged_only','spheres','off','projlines','off');
    fig_i = get(groot,'CurrentFigure');
...
%     saveas(gcf,fullfile(cluster_dir,'Cluster_dipole_plot_allaveraged_only.jpg'));
    ...
    STUDY.etc.dipparams.centrline = 'off';
    std_dipplot_CL(STUDY,ALLEEG,'clusters',CLUSTER_PICKS,'figure','off','mode','together_averaged_multicolor','spheres','off','projlines','off');
    fig_i = get(groot,'CurrentFigure');
    set(fig_i,'position',[16 582 500 300],'color','w')
    camzoom(1.2^2);
...
%         parfor (cond_i = 1:length(CLUSTER_PICKS),POOL_SIZE)
%             cluster_i = CLUSTER_PICKS(cond_i);
%             % NOTE: (07/20/2023) DOES NOT WORK... if you want stats on the plot, too!!
%             [~, spec_data, ~, spec_freqs, ~, params_ersp] = std_readdata(STUDY,ALLEEG,...
%                             'clusters',cluster_i,'singletrials',SPEC_PARAMS.singletrials,... 
%                             'datatype','spec','freqrange',SPEC_PARAMS.freqrange,...
%                             'design',des_i);
%             %- across condition plot
%             alltitles = std_figtitle('condnames', COND_DESIGNS{des_i},...
