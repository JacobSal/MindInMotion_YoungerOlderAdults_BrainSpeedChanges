...
for clust_i = MIN_ICS:mean_IC_allSub
    ...
    %- Plot topographies 
    figure;
    std_topoplot_CL(STUDY,main_cl_inds,'together');
    ...
    std_dipplot_CL(STUDY,ALLEEG,'clusters',main_cl_inds,'figure','off','mode','together_averaged');
    set(gcf,'position',[16 582 1340 751],'color','w')
...
%             stacked_cond_ersp = [allersp{:}];
    ...
    %             tmp = repmat(tmp,1,size(allersp{cond_i},2));
            ...
            saveas(fig_i,[cluster_dir filesep sprintf('custom2_Cond_ersp_%i_%s.fig',cluster_i,[COND_DESIGNS{des_i}{:}])]);
            saveas(fig_i,[cluster_dir filesep sprintf('custom2_Cond_ersp_%i_%s.jpg',cluster_i,[COND_DESIGNS{des_i}{:}])]);
            close(fig_i)
        end
    end
