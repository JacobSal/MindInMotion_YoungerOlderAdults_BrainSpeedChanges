...
conn_meas = 'dDTF08';
load_trials = {'flat','low','med','high'}; %{'rest','0p25','0p5','0p75','1p0','flat','low','med','high'};
%## soft define
%- path for local data
DATA_DIR = [source_dir filesep '_data'];
STUDIES_DIR = [DATA_DIR filesep DATA_SET filesep '_studies'];
subjinfDir = [SUBJINF_DIR filesep sprintf('%s',dt)];
save_dir = [STUDIES_DIR filesep sprintf('%s',dt) filesep '_figs' filesep 'connectivity'];
        ...
        for subj_i = 1:length(MAIN_ALLEEG)
            ...
            if any(chk)
                fprintf('%s) Frequencies: %sHz\n',MAIN_ALLEEG(subj_i).subject,...
                    FREQS_SUBPATH);
                statMat = MAIN_ALLEEG(subj_i).etc.js_processing(sub_cond_i).META.(conn_meas).connExtract(freq_i).mats;
