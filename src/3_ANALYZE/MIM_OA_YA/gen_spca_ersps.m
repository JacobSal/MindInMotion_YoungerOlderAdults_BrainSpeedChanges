...
% sbatch /blue/dferris/jsalminen/GitHub/par_EEGProcessing/src/3_ANALYZE/MIM_OA_YA/run_gen_spca_ersps.sh
...
parfor (subj_i = 1:length(MAIN_ALLEEG),floor(length(MAIN_ALLEEG)/3))
...
%         EEG_gait = eeglab_pop_subcomp(EEG_gait,(1:4),0);
        ...
        par_save(GPM_corr,fpath,sprintf('cond%s_spca_ersp.mat',SPCA_PARAMS.condition_gait{cond_i}));
        %##
        fig = plot_txf(squeeze(ERSP_corr(:,CHAN_INT,:)),[],WAVELET_STRUCT.f);
...
%%
for subj_i = 1:length(MAIN_ALLEEG)
    ERSP_corr = par_load(fpath,sprintf('cond%s_spca_ersp.mat',SPCA_PARAMS.condition_gait{cond_i}));
    clusterinf = STUDY.cluster;
    for cl_i = 1:length(clusterinf)
        ind = (clusterinf(cl_i).sets == subj_i);
        comp = clusterinf(cl_i).comps(ind);
