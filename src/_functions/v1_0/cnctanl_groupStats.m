function [EEG,cfg] = cnctanl_groupStats(EEG,PATHS,stat,varargin)
...
addParameter(p,'VERBOSITY_LEVEL',Defaults{2}, @isnumeric)
addParameter(p,'savePath',Defaults{3})
parse(p, EEG, PATHS, stat, varargin{:});

GUI_MODE = p.Results.GUI_MODE;
VERBOSITY_LEVEL = p.Results.VERBOSITY_LEVEL;
...
switch stat
    ...
    case 'PhaseRnd'
        ...
        cfg = [];
        cfg.mode = {'PhaseRand','nperms',N_PERMS};
%         cfg.PhaseRand.nperms  = 200;
%         cfg.nperms = 200;
%         cfg.PhaseRand.nperms = 200;
        cfg.verb = 1;
        [PConn, cfg] = feval(@stat_surrogateGen,'ALLEEG',EEG,cfg);
...
%% SUBFUNCTIONS
% getSubPath.f
function [pathOut,lcl] = getSubPath(subF,PATHS,savePath)
    lcl = [];
    if isempty(savePath)
        error('Please define ''savePath''');
    else
        lcl = savePath;
        if ~isfolder([lcl filesep subF])
            fprintf('Making new folder...\n')
            mkdir([lcl  filesep subF])
        end
        pathOut = [lcl  filesep subF];
    end
end
end
