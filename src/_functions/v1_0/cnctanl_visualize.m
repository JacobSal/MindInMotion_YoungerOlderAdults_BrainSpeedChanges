function [cfg,handles] = cnctanl_visualize(EEG,PATHS,components,clusters,visual,varargin)
...
addRequired(p,'components')
addRequired(p,'clusters')
...
parse(p, EEG, PATHS, components, clusters, visual, varargin{:});
%## SET DEFAULTS
...
%% ===================================================================== %%
if isempty(clusters)
    clusters = repmat(0,length(components),1);
end

if isempty(componentNames)
    ComponentNames = componentNames;
    for j = 1:length(components)
        ComponentNames = [ComponentNames, {[clusters{j} '_(' num2str(components(j)) '-' num2str(j) ')']}];
    end
else
    ComponentNames = componentNames;
end
...
%         [EEG,cfg,handles] = pop_vis_TimeFreqGrid(EEG,typeproc,cfg);
    ...
    case 'exTimeFreqChart'
        ...
        for cnd = 1:length(connMeasures)
            ...
            cfg.vismode = 'TimeXFrequency';
            cfg.msubset = 'all';
            cfg.thresholding = THRESH_CHAR;
            if ~strcmp(connMeasures{cnd},'mCoh')
                cfg.Stats = EEG.CAT.Stats;
                cfg.Stats.plotci = false;
%                 cfg.Stats.sigthreshmethod = 'pval';
...
%                         subargs.thresholding.alpha = 0.01;
                    ...
                    if isempty(clim)
                        clim = [mn-(stdv),mn+(stdv)];
                    end
                    fprintf('Range: %0.3f\n',rnge);
                    fprintf('Mean: %0.3f\n',mn);
                    fprintf('Standard Deviation: %0.3f\n',stdv);
                    fprintf('Median: %0.3f\n',med);
                    fprintf('1st, 25th, 75th, and 99th Percentile: %0.3f, %0.3f, %0.3f, %0.3f\n',prc(1),prc(2),prc(3),prc(4));
                    fprintf('Min & Max: %0.3f & %0.3f\n',min(tmp),max(tmp));
                    %- topoplot plotting
                    ...
                    subargs.linewidth = 3;
                    subargs.titleString = sprintf('%s) component %i->%i',EEG.subject,i,j);
...
%                     fig.Position = [0.025 0.05 0.95 0.85];
                    ...
                    saveas(fig,[savePath filesep sprintf('cell_Comp-%i-%i_%s.fig',i,j,connMeasures{cnd})]);
                    saveas(fig,[savePath filesep sprintf('cell_Comp-%i-%i_%s.png',i,j,connMeasures{cnd})])
