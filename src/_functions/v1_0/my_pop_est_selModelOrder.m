function [ALLEEG cfg] = my_pop_est_selModelOrder(ALLEEG,typeproc,varargin)
%
...
fcnName     = strrep(mfilename,'my_pop_',''); % (05/19/2022) change to 'my_pop'
...
% (05/19/2022) added bypass to remove GUI prompt during scripting.
if ~strcmpi(typeproc,'nogui')
...
if strcmpi(res,'yes')
    ...
    % Once model is fit results will be return in EEG structure
    modFuncName = ['pop_' ALLEEG(1).CAT.IC.modelFitting.modelingFuncName];
    ALLEEG = feval(modFuncName, ALLEEG,'nogui',ALLEEG(1).CAT.IC.modelFitting.modelingArguments);
end
