function [ALLEEG] = cnctanl_connMeas(ALLEEG,components,connMeasures,varargin)
%   Project Title: 
...
%## OPTIONAL
addOptional(p,'fig_savepath',[],validPath)
%## PARAMETER
...
parse(p, ALLEEG, components, connMeasures, varargin{:});
...
%## PARAMS
MORDER_RANGE            = [1, ((ALLEEG(1).srate*WINDOW_LENGTH)/2-1)];
MORDER                  = ((ALLEEG(1).srate*WINDOW_LENGTH)/2-1);
...
for cond_i = 1:length(ALLEEG)
    ...
    saveas(handles,[fig_savepath filesep sprintf('%s_orderResults.fig',ALLEEG(cond_i).subject)]);
    close(handles);
end
...
for cond_i = 1:length(ALLEEG)
    ...
    % If you want to save this figure you can uncomment the following lines:
    saveas(handles,[fig_savepath filesep sprintf('%s_validationResults.fig',ALLEEG(cond_i).subject)]);
    close(handles);
