...
% Edit by Chang Liu, 2021-11
function clustering_solutions = repeated_clustering(STUDY,ALLEEG, n_iterations, n_clust, outlier_sigma,preclustparams)
...
fprintf('Clustering %d times...\n', n_iterations)
    ...
    for iteration = 1:n_iterations
        ...
        [STUDY_new] = pop_clust_CL(STUDY, ALLEEG, 'algorithm','kmeans','clus_num',  n_clust , 'outliers',  outlier_sigma );   %Note that if the pop_clust uses robust_kmeans which iterates the cluster for 30 iterations      
        ...
        clustering_solutions.(['solution_' num2str(iteration)]) = STUDY_new.cluster;
        % Note by Chang: To minimize the usage of memory
        clear STUDY_new;
