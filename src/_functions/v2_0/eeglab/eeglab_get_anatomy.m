...
chk = zeros(length(cluster_inds),7);
for i = 1:length(cluster_inds)
    cl_i = cluster_inds(i);
    chk(i,1) = isfield(STUDY.cluster(cl_i),'topox') && ~isempty(STUDY.cluster(cl_i).topox);
    chk(i,2) = isfield(STUDY.cluster(cl_i),'topoy') && ~isempty(STUDY.cluster(cl_i).topoy);
    chk(i,3) = isfield(STUDY.cluster(cl_i),'topoall') && ~isempty(STUDY.cluster(cl_i).topoall);
    chk(i,4) = isfield(STUDY.cluster(cl_i),'topo') && ~isempty(STUDY.cluster(cl_i).topo);
    chk(i,5) = isfield(STUDY.cluster(cl_i),'topopol') && ~isempty(STUDY.cluster(cl_i).topopol);
    chk(i,6) = isfield(STUDY.cluster(cl_i),'all_diplocs') && ~isempty(STUDY.cluster(cl_i).all_diplocs);
    chk(i,7) = isfield(STUDY.cluster(cl_i),'dipole') && ~isempty(STUDY.cluster(cl_i).dipole);
    % fprintf('%i\n',chk);
