function [psd_out,gait_avg,output_struct] = apply_spca_cond_psd(f_mat_struct,base_f_mean,varargin)
...
%##
% gait_txf = zeros(size(test,3),size(test,2),length(inds),size(test,1),'single');
gait_f = zeros(size(test,1),size(test,2),length(inds),'double');
for i = 1:length(inds)
    gait_f(:,:,i) = f_mat_struct.(fn{inds(i)}); % freq x epoch x chan
end

if ~isempty(COND_STR)
    inds = strcmp({f_mat_struct.trialinfo.cond},COND_STR);
    gait_f = squeeze(gait_f(:,inds,:));
end
%- average across trials
gait_f = permute(gait_f,[2,1,3]);
gait_avg = mean(gait_f); % average over trials and take abs() to ensure amplitude calc
gait_avg = permute(gait_avg,[1,3,2]);
% gait_avg = permute(gait_avg,[]);
%-
psd_baselined = bsxfun(@minus,gait_avg,base_f_mean);
...
if ~isempty(COEFFS)
    ...
    output_struct = struct('baseline_ersp',base_f_mean,...
        'psc1',PSC1,...
        'erds_orig',psd_baselined,...
        'gpm_orig',GPM);
else
