...
%     EEG.event = struct('type',[],'latency',[],'urevent',[],'duration',[]);
    EEG = ES_epoch_data(EEG);
    tmp = strcmp({EEG.event.(EVENT_COND_PARSER)},);
    ...
    TMP_EEG.condition = conditions{cond_i};
    %- assign values to output structure
    pathsout(cond_i).epoch.label = sprintf('Epoch_%s-%s',PARSE_TYPE,conditions{cond_i});
    pathsout(cond_i).epoch.filepath = fPath;
    pathsout(cond_i).epoch.filename = sprintf('%s_%s_EPOCH_TMPEEG.set',TMP_EEG.subject,conditions{cond_i});
    pathsout(cond_i).META.parseVariable = PARSE_TYPE;
    %* check to make sure a number isn't the first character
    chk = regexp(conditions{cond_i},'\d');
    if any(chk)
        conditions{cond_i} = sprintf('x%s',conditions{cond_i});
    end
    pathsout(cond_i).META.condition = conditions{cond_i};
...
for conn_i = 1:length(CONN_METHODS)
    ...
    for cond_i = 1:length(ALLEEG)
        ...
        [statMat,extract_sig] = gen_connMatrix(ALLEEG(cond_i),CONN_METHODS{conn_i},1:length(ALLEEG(cond_i).CAT.Conn.freqs),STAT_ALPHA);
        pathsout(cond_i).META.(CONN_METHODS{conn_i}).connExtract(1).freqs = ALLEEG(cond_i).CAT.Conn.freqs;
        pathsout(cond_i).META.(CONN_METHODS{conn_i}).connExtract(1).mats = statMat;
        pathsout(cond_i).META.(CONN_METHODS{conn_i}).connExtract(1).sigs = extract_sig;
    end
end
...
if true
    for cond_i = 1:length(ALLEEG)
        ...
        ALLEEG(cond_i).etc.js_processing = pathsout;
        [~,~] = pop_saveset(ALLEEG(cond_i),...
            'filepath',ALLEEG(cond_i).filepath,'filename',ALLEEG(cond_i).filename,...
            'savemode','twofiles');
    end
