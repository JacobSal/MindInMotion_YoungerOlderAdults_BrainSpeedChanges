...
for cond_i = 1:length(conditions)
    ...
    if strcmp(cond_name,'rest')
        [tmp] = MIM_epochData(EEG,PARSE_TYPE,...
        ...
        try
            tmp = pop_epoch(EEG,{EVENT_CHAR},EPOCH_TIME_LIMITS,'eventindices',valid_points(1):valid_points(2),'epochinfo','yes');
        catch
            error('ERROR. Something went wrong when epoching subject %s',EEG.subject);
        end
    end
    ...
    pathsout(cond_i).epoch.filename = sprintf('%s_%s_EPOCH_TMPEEG.set',tmp.subject,cond_name);
    ...
    [tmp] = pop_saveset(tmp,'savemode','twofiles',...
...
if DO_PHASE_RND
    ...
    for cond_i=1:length(ALLEEG)
        ...
        tmp = ALLEEG(cond_i).CAT.PConn;
        fName = strsplit(ALLEEG(cond_i).filename,'.'); fName = [fName{1} '.mat'];
        par_save(tmp,ALLEEG(cond_i).filepath,fName,[],'_PhaseRnd');
        fprintf('done.\n')
        %{
...
for conn_i = 1:length(CONN_METHODS)
    ...
    for cond_i = 1:length(ALLEEG)
        ...
        freqs = 1:length(ALLEEG(cond_i).CAT.Conn.freqs);
        %- calculate average connnectivity for each component pair across time
        [statMat,extract_sig] = gen_connMatrix(ALLEEG(cond_i),CONN_METHODS{conn_i},freqs,STAT_ALPHA);
