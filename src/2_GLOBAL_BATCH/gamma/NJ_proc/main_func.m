...
CONN_METHODS = {'ffDTF','dDTF','GGC'}; % Options: 'S', 'dDTF08', 'GGC', 'mCoh', 'iCoh'
STAT_ALPHA = 0.01; % 
% subj_i_genConnMeas
FREQS = (3:60);
...
for cond_i = 1:length(conditions)
    ...
    %## EPOCH SUBJ DATA
    %- fix some NJ event stuff
    if strcmp(cond_name,'pre')
        tmp = pop_epoch(EEG,{'pre'},EPOCH_TIME_LIMITS);
    elseif strcmp(cond_name,'post')
        tmp = pop_epoch(EEG,{'post'},EPOCH_TIME_LIMITS);
    end
