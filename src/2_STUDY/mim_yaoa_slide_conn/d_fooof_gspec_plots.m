...
xlsx_fpath = [STUDIES_DIR filesep sprintf('%s',cluster_study_dir) filesep 'raw_data_vis'];
...
REGRESS_TXT_SIZE = 8;
REGRESS_TXT_XMULTI = 0.5;
REGRESS_TXT_YMULTI = 1.4;
STATS_OUT = [];
im_resize= 0.9;
VIOLIN_BOTTOM = 0.7;
AX_H  = 0.2;
...
for var_i = 1:length(varnames)
   ...
   for des_i = 2 %## JUST SPEED
       ...
       try
        ...
        hold off;
        exportgraphics(fig,[kin_savedir filesep sprintf('%s_kinematics-speed.tiff',varnames{var_i})],'Resolution',300)
        close(fig)
        %- iterate
   end
end
...
%% PREDICTORS: KINEMATICS, COND. RESPONSE: BRAIN ACTIVITY, STATS TEST
REGRESS_TXT_SIZE = 8;
REGRESS_TXT_XMULTI = 0.9;
REGRESS_TXT_YMULTI = 1.0;
im_resize = 0.7;
    ...
    for var_i = 1:length(varnames)
        ...
        for des_i = 2
            ...
            for meas_i = 1:length(measure_name_plot)
                ...
                y_txt = max(T_vals_plot.(measure_name))*REGRESS_TXT_YMULTI+std(T_vals_plot.(measure_name));
                try
                    mod = sprintf('%s ~ 1 + %s + cond_char',measure_name,varnames{var_i});
                    stats_out = fitlm(T_vals_plot,mod);
                    anova_out = anova(stats_out);
                    R2 = stats_out.Rsquared.Adjusted;
                    anova_p_var = anova_out.pValue(strcmp(anova_out.Properties.RowNames,varnames{var_i}));
