...
%% ================================================================= %%
%## COMBINE GROUPS
% store_spec_orig = spec_data_original;
% tmp_sporig = cell(size(spec_data_original));
% store_spec_diff = fooof_diff_store;
% tmp_spdiff = cell(size(fooof_diff_store));
% for k_i = 1:length(clusters)
%     cl_i = double(string(clusters(k_i)));
%     for des_i = 1:length(designs)
%         for i = 1:size(spec_data_original{des_i}{cl_i},1)
%             data = cat(2,spec_data_original{des_i}{cl_i}{i,:});
%             tmp_sporig{des_i}{cl_i}{i,1} = data;
%             data = cat(2,fooof_diff_store{des_i}{cl_i}{i,:});
%             tmp_spdiff{des_i}{cl_i}{i,1} = data;
%         end
%     end
% end
% fooof_diff_store = tmp_spdiff;
% spec_data_original = tmp_sporig;
%## PSDS
PSD_BOTTOM = 0.7;
im_resize = 0.5;
AXES_DEFAULT_PROPS = {'box','off','xtick',[],'ytick',[],'ztick',[],'xcolor',[1,1,1],'ycolor',[1,1,1]};
    ...
    for des_i = 1:length(designs)   
        ...
        for i = 1:length(measure_name_plot)
            ...
            PLOT_PARAMS = struct('color_map',color_dark,...
                ...
                'cond_offsets',[-0.3,-0.1,0.1,0.3],'group_offsets',[0.5],...
...
%         im3(1,:,:) = [];
    ...
    for des_i = 1:length(designs)   
        ...
        for i = 1:length(measure_name_plot)
            ...
            PLOT_PARAMS = struct('color_map',color_dark,...
                'cond_labels',unique(T_plot.cond_char),'group_labels',g_char,...
                'cond_offsets',[-0.3,-0.1,0.1,0.3],'group_offsets',[0.5],...
                'y_label','10*log_{10}(Flattened PSD)',...
                'title',title_plot{i},'font_size',PLOT_STRUCT.font_size,'ylim',[min(T_plot.(measure_name))-0.3,max_vals(i)],...
                'font_name',PLOT_STRUCT.font_name,'x_label',x_label,'do_combine_groups',false);
            axax = group_violin(T_plot,measure_name,'cond_id','group_id',...
                fig,...
                'VIOLIN_PARAMS',VIOLIN_PARAMS,...
                'PLOT_PARAMS',PLOT_PARAMS,...
