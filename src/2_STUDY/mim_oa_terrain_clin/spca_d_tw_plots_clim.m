...
allfreqs = hardcode_freqs(freq_crop);
PLOT_STRUCT_PAR = PLOT_STRUCT;
PLOT_STRUCT_PAR.freq_lims = [allfreqs(1),allfreqs(end)];
PLOT_STRUCT_PAR.time_lims = [alltimes(1),alltimes(end)];
%##
con_con = cond_iters{des_i};
erspcorr = cell(length(con_con),1); 
pc1 = cell(length(con_con),1); 
subjs = zeros(length(con_con),1);
trial_order = cell(length(con_con),1);
for cond_i = 1:length(con_con)
    %- get cluster & condition indices
    ...
    tmp = cat(3,spca_table.tf_ersporig_c{inds});
    tmp = permute(tmp,[2,1,3]);
    ersporig{cond_i} = tmp; %tmp(freq_crop,time_crop);
    %- extract erspcorr
    tmp = cat(3,spca_table.tf_erspcorr_c{inds});
    tmp = permute(tmp,[2,1,3]);
    erspcorr{cond_i} = tmp; %tmp(freq_crop,time_crop);
    %- extract pc1
    tmp = cat(3,spca_table.tf_pc1_c{inds});
    tmp = permute(tmp,[2,1,3]);
    pc1{cond_i} = tmp; %tmp(freq_crop,time_crop);
end
COMMON_BASE = false;
SUBJ_BASE = true;
[~,ersporig,~,~] = eeglab_baseln(ersporig,hardcode_times,hardcode_freqs,...
            TIME_BOUND,FREQ_BOUND,...
            'DO_COMMON_BASE',COMMON_BASE,...
            'DO_SUBJ_BASE',SUBJ_BASE);
[~,erspcorr,~,~] = eeglab_baseln(erspcorr,hardcode_times,hardcode_freqs,...
            TIME_BOUND,FREQ_BOUND,...
            'DO_COMMON_BASE',COMMON_BASE,...
            'DO_SUBJ_BASE',SUBJ_BASE);
[~,pc1,~,~] = eeglab_baseln(pc1,hardcode_times,hardcode_freqs,...
            TIME_BOUND,FREQ_BOUND,...
            'DO_COMMON_BASE',COMMON_BASE,...
            'DO_SUBJ_BASE',SUBJ_BASE);
%% PLOT
...
subj_i = 1;
for cond_i = 1:length(con_con)
    p1{cond_i} = squeeze(mean(ersporig{cond_i},3));
    p2{cond_i} = squeeze(mean(erspcorr{cond_i},3));
    p3{cond_i} = squeeze(mean(pc1{cond_i},3));
    %-
    % p1{cond_i} = ersporig{cond_i}(:,:,subj_i);
    % p2{cond_i} = erspcorr{cond_i}(:,:,subj_i);
    % p3{cond_i} = pc1{cond_i}(:,:,subj_i);
end
%##
if any(strcmp(trial_order,SPEED_REF_CHAR))
    condnames = SPEED_OVERRIDE_CHARS;
...
ERSP_CLIM = [-0.4,0.4];
% ERSP_CLIM = [-2.4,2.4];
PLOT_STRUCT_PAR.font_size = 8;
PLOT_STRUCT_PAR.alltitles = alltitles(cond_i);
PLOT_STRUCT_PAR.do_display_bandmarks = false;
PLOT_STRUCT_PAR.clim = ERSP_CLIM;
PLOT_STRUCT_PAR.figure_title = 'All PCs ERSP';
fig = plot_txf_conds_tftopo(p1(cond_i),alltimes,allfreqs,[],{},...
    'PLOT_STRUCT',PLOT_STRUCT_PAR);
drawnow;
exportgraphics(fig,[save_dir filesep sprintf('cl%i_des%i_spca_ersporig_ex.tiff',cl_i,des_i)],'Resolution',300);
%##
% ERSP_CLIM = [-0.5,0.5];
% PLOT_STRUCT_PAR.font_size = 8;
PLOT_STRUCT_PAR.alltitles = alltitles(cond_i);
PLOT_STRUCT_PAR.clim = ERSP_CLIM;
PLOT_STRUCT_PAR.do_display_bandmarks = false;
PLOT_STRUCT_PAR.figure_title = 'PCs 2-N ERSP';
fig = plot_txf_conds_tftopo(p2(cond_i),alltimes,allfreqs,[],{},...
    'PLOT_STRUCT',PLOT_STRUCT_PAR);
drawnow;
exportgraphics(fig,[save_dir filesep sprintf('cl%i_des%i_spca_erspcorr.tiff',cl_i,des_i)],'Resolution',300);
%##
% ERSP_CLIM = [-0.4,0.4];
% PLOT_STRUCT_PAR.font_size = 8;
PLOT_STRUCT_PAR.alltitles = alltitles(cond_i);
PLOT_STRUCT_PAR.clim = ERSP_CLIM;
PLOT_STRUCT_PAR.do_display_bandmarks = false;
PLOT_STRUCT_PAR.figure_title = '1st PC ERSP';
fig = plot_txf_conds_tftopo(p3(cond_i),alltimes,allfreqs,[],{},...
    'PLOT_STRUCT',PLOT_STRUCT_PAR);
drawnow;
exportgraphics(fig,[save_dir filesep sprintf('cl%i_des%i_spca_pc1_ex.tiff',cl_i,des_i)],'Resolution',300);
%}