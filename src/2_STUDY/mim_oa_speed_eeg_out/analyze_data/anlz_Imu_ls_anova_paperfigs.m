...
[p,t,anova_out,terms] = anovan(tmp_table.(meas_names{meas_i}),{tmp_table.(GROUP_TABLE_VAR)},...
    'sstype',3,'varnames',{'trial_tag_in',GROUP_TABLE_VAR},'model','linear','Display','off');
...
[p,t,anova_out,terms] = anova(altstats_out);
%- alternative f2?
...
VIOLIN_PARAMS = {'width',0.1,...
    ...
    'HalfViolin','full','DataStyle','scatter','MarkerSize',12,...
...
ax.Children(1).Position(2) = -0.15;
ax.Children(2).Position(2) = -0.15;
ax.Children(3).Position(2) = -0.15;
yt = yticks(ax);
...
[p,t,anova_out,terms] = anovan(tmp_table.(meas_names{meas_i}),{tmp_table.trial_char, tmp_table.(GROUP_TABLE_VAR)},...
    'sstype',3,'varnames',{'trial_char',GROUP_TABLE_VAR},'model','linear','Display','off');
[comparisons,means,~,gnames] = multcompare(anova_out,'Dimension',[2],...
...
PLOT_STRUCT = struct('color_map',COLOR_MAPS_SPEED,...
    'cond_labels',{xtick_label_g},'group_labels',{{'YA','OHMA','OLMA'}},...
...
%     'bracket_y_perc',0.8,...
%     'bracket_rawshifty_upper',1,...
%     'bracket_rawshifty_lower',0,...
%     'grp_sig_offset_x',0,...
%     'grp_sig_offset_y',1);
STATS_STRUCT = struct('anova',{{anova_p,anova_p,anova_p}},...
    'anova_grp',{{}},...
    'pvals',{num2cell(adj_p)},...
...
%## RANOVA
% tt_out = unstack(tmp_table,meas_names{meas_i},'trial_char');
% mod_out = sprintf('x0_25,x0_5,x0_75,x1~1+%s',GROUP_TABLE_VAR);
% mod_out = sprintf('%s~1+%s+trial_char',meas_names{meas_i},GROUP_TABLE_VAR);
% tmptmp = table([1,2,3,4]','VariableNames',{'speed'});
% %-
% rm_out = fitrm(tt_out,mod_out,'WithinDesign',tmptmp);
...
% [ran_out,A,C,D] = ranova(rm_out);
% [tbl] = rm_out.multcompare(GROUP_TABLE_VAR);
%-
disp(stats_out)
%- test normality
[norm_h,norm_p] = lillietest(stats_out.Residuals.Raw);
