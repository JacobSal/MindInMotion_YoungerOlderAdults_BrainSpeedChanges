...
T_DIM = size(spca_psd.psd_corr,1);
F_DIM = size(spca_psd.psd_corr,3);
        ...
        for cond_i = 1:length(condition_gait)
            ...
            for i = 1:length(CLUSTER_PICKS)
                ...
                if ~isempty(comp_i) && length(comp_i) < 2
                    ...
                    psdcorr_c{cnt} = squeeze(spca_psd.psd_corr(:,tmp_c,:));
...
%                                     'ic_keep',ic_keep(comp_i));
        ...
        loop_store{subj_i} = table(subj_c,group_c,cluster_c,cond_c,comp_c,psdcorr_c,psdorig_c,tf_pc1_c,tf_coeff_c);
