...
parfor subj_i = 1:length(ALLEEG)
    ...
    %##
    if DO_RECREATE_SPCA || ~all(cellfun(@(x) exist([EEG.filepath filesep sprintf('cond%s_spca_psd.mat',x)],'file'),TRIAL_TYPES))
        %## GENERATE ERSPS
        ...
        if ~exist(icaspec_f,'file') || FORCE_RECALC_PSD % || any(strcmp(EEG.subject,FINISHED_ADULTS))
            ...
            fprintf('Done calculating timewarped PSDs for %s\n',EEG.subject);
        else
            fprintf('Timewarped PSDs already calculated for %s\n',EEG.subject);
        end
