...
<!-- ci = 9; -->
<!-- tmpt <- filter_at(eegt,vars('cluster_id'), any_vars(. %in% clusters[ci])); -->
<!-- fit = lmer(str_glue("theta_avg_power ~ 1 + speed_cond_num + group_char + speed_cond_num:group_char + (1|subj_char)"), data=tmpt) -->
...
<!--   t_out <- as_kable_extra(tbl, format = "HTML") %>% -->
<!--     add_header_above(c("Cluster: ",clusters[ci])) %>% -->
<!--     kable_styling(latex_options="scale_down") -->
<!--   # tbl -->
