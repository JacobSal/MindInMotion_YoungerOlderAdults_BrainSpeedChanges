...
excel_dir <-"M:/jsalminen/GitHub/par_EEGProcessing/src/_data/MIM_dataset/_studies/04232024_MIM_YAOAN89_antsnorm_dipfix_iccREMG0p4_powpow0p3_skull0p01_15mmrej/cluster/icrej_5/12/spec_data/group_spec/psd_calcs/fooof_kinematics_table.xlsx";
...
eeg_measures = c('theta_avg_power','alpha_avg_power','beta_avg_power','beta_div_theta','theta_div_beta','aperiodic_exp','aperiodic_offset');
```
...
for (ci in 1:(length(clusters))) {
  ...
  for (mi in 1:length(kin_measures)) {
    tbl <- tibble(outcome=eeg_measures[4:5]) %>%
      rowwise() %>%
      mutate(
        tbl = 
      ...
      pull(tbl) %>% 
      tbl_merge(tab_spanner=c("**Beta Div Theta**","**Theta div Beta**"))
...
<!-- \clearpage -->
# LME EEG ~ 1+kin+group
```{r EEG ~ 1+kin+group+kin:group2, echo=FALSE, message=FALSE, results="asis"}
library(gtsummary)
cat('\n\n<!-- -->\n\n')
for (ci in ((length(clusters)/2)+1):length(clusters)) {
  tmpt <- filter_at(eegt,vars('cluster_id'), any_vars(. %in% clusters[ci]));
  for (mi in 1:length(kin_measures)) {
    tbl <- tibble(outcome=eeg_measures[4:5]) %>%
      rowwise() %>%
