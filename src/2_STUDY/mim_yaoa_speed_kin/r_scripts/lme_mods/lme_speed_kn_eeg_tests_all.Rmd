...
# KINEMATIC TESTS: SPEED, KIN, INTERACTION
...
# EEG TESTS: KINEMATIC, SPEED, INTERACTION

## MEDIATION) LME EEG ~ 1+kin+speed+kin:speed
...
for (ci in cluster_inds_plot) {
  ...
  for (mi in 1:length(kin_measures)) {
    ...
    for(modi in 1:length(eeg_measures[eeg_inds_plot])){
      ...
      plot_ly(x=tmpt[[kin_measures[mi]]], y=tmpt[[eeg_measures[eegi]]], z=tmpt$speed_cond_num, type="scatter3d", mode="markers", color=tmpt[[kin_measures[mi]]])
      
      m <- matrix(vals, nrow = length(unique(d$wt)), ncol = length(unique(d$disp)))
      plot_ly() %>% add_surface((x=.data[[kin_measures[mi]]],y=tmpt[[eeg_measures[eegi]]],z=tmpt$speed_cond_num))
