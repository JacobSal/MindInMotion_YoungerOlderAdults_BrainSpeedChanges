...
%         folder_to = [M_MIND_IN_MOTION_DIR filesep SUBJ_PICS{group_i}{subj_i} filesep 'MRI' filesep 'HeadModel' filesep 'skull_0p01'];
        ...
        if exist(file_from,'file')&&~exist([folder_to filesep 'elec_aligned_init.mat'],'file')
%             fprintf('%s) copying file %s',SUBJ_PICS{group_i}{subj_i},file_from);
            copyfile(file_from,folder_to)
        else
            fprintf('%s) File does not exist or is already transfered: %s\n',SUBJ_PICS{group_i}{subj_i},file_from);
        end
        ...
        if exist(file_from,'file')&&~exist([folder_to filesep 'elec_aligned.mat'],'file')
%             fprintf('%s) copying file %s',SUBJ_PICS{group_i}{subj_i},file_from);
            copyfile(file_from,folder_to)
        else
            fprintf('%s) File does not exist or is already transfered: %s\n',SUBJ_PICS{group_i}{subj_i},file_from);
        ...
        if exist(file_from,'file')&&~exist([folder_to filesep 'vol.mat'],'file')
%             fprintf('%s) copying file %s',SUBJ_PICS{group_i}{subj_i},file_from);
            copyfile(file_from,folder_to)
        else
            fprintf('%s) File does not exist or is already transfered: %s\n',SUBJ_PICS{group_i}{subj_i},file_from);
        ...
        if exist(tmp,'file')&&~exist([folder_to filesep 'CustomElectrodeLocations.txt'],'file')
